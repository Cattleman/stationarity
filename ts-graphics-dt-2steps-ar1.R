##
## Description: Plot the results of 2 steps decision tree stationarity tests
##
...
for(error in ERROR_OPTION){
  for(p in phis){
    for(q in thetas){
      ...
      ggsave(paste0(outputDir,"/dt-2steps-arma-", error, "/stationary-ar1-" , p, "-",
                    error, ".png"),
             pl, width=20, height=9.5)
    }
  }
}
  ...
  if((type == "M") | (type == "V")){
    for(tindex in 1:length(tests)){
      for(phi in 1:length(phis)){
        ...
        ggsave(paste0(outputDir, "/dt-2steps-trend/trend", type, "-phi",
                      phis[phi], "-", tests[tindex], ".png"),
    ...
    for(tindex in 1:length(tests)){
      ...
      pl <- ggplot(data[which(window <= 4096)], aes(x=as.numeric(as.character(window)),
        ...
        ggtitle(substitute(paste(t, "     ", p),
                           list(t=tests[tindex], p=values[value]))) +
        scale_y_continuous(name="% Detected Stationarity",
                           breaks=c(0, 20, 40, 60, 80, 100),
                           labels=c("0%", "20%", "40%", "60%", "80%", "100%"),
                           limits=c(-10, 100)) +
      ...
      ggsave(paste0(outputDir, "/dt-2steps-trend/trend", type, "-",
                    tests[tindex], ".png"),
             pl, width=9.86, height=5.43)
    }
  }
	...
	if ((type == "M") || (type == "V")){
		for(tindex in 1:length(tests)){
			for(phi in 1:length(phis)){
				...
				ggsave(paste0(outputDir, "/dt-2steps-break/break", type, "-phi", phis[phi], "-",
		...
		for(tindex in 1:length(tests)){
			...
			ggsave(paste0(outputDir, "/dt-2steps-break/break", type, "-", tests[tindex], ".png"),
					pl, width=9.86, height=5.43)
		}
	}
}
