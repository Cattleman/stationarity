##
## Description: Execute 3 steps decision tree stationarity tests on different
...
if(PARAM_OPTION[paramID] == PARAM_ARMA){
  ...
  error <- ERROR_OPTION[errorID]
	...
	## Create directory
  tsOutputDir <- paste0(outputDir, "/dt-3steps-", PARAM_OPTION[paramID], "-",
  ...
  for(tindex in 1:length(tests)){
    for(windex in 1:length(windows)){
      ...
      aux <- foreach(n=1:N, .combine=c) %dopar%
        decisionTree3Steps(ts[1:windows[windex], n], alpha, tindex)[[1]]
  ...
  fname <- paste0(tsOutputDir, "/armaTime-p", phi, "-q", theta, "-",
                  error, ".csv")
  if (file.exists(fname)){
    write.table(x, file=fname, sep=";", quote=FALSE,
