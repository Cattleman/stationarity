##
## Description: Summarizes the results of the stationarity algorithm
##
...
for(error in ERROR_OPTION){
  ...
  write.table(summary, file=paste0(outputDir, "/algo-arma-", error, "-summary.csv"),
...
for(error in ERROR_OPTION){
  ...
  for(p in phis){
    for(q in thetas){
      ...
      data <- NULL
      armaTime <- read.csv(paste0(inputDir, "/algo-arma-", error,
  ...
  write.table(summary, file=paste0(outputDir, "/algo-arma-", error, "-time.csv"),
              append=FALSE, quote=FALSE, sep=";", row.names=FALSE,
              col.names=TRUE)
}
...
write.table(summary, file=paste0(outputDir, "/algo-trend-summary.csv"),
            append=FALSE, quote=FALSE, sep=";", row.names=FALSE,
            col.names=TRUE)
...
for(type in types){
  ...
  if((type == "M") | (type == "V")){
    for(phi in 1:length(phis)){
      for(value in 1:length(values)){
        armaTime <- read.csv(paste0(inputDir,"/algo-trend/trend", type,
...
write.table(summary, file=paste0(outputDir, "/algo-trend-time.csv"),
            append=FALSE, quote=FALSE, sep=";", row.names=FALSE,
            col.names=TRUE)
...
for(type in types){
  ...
  if((type == "M") || (type == "V")){
    for(tindex in 1:length(tests)){
      for(phi in 1:length(phis)){
        for(value in 1:length(values)){
          armaResults <- read.csv(paste0(inputDir, "/algo-break/break",
...
write.table(summary, file=paste0(outputDir, "/algo-break-summary.csv"),
            append=FALSE, quote=FALSE, sep=";", row.names=FALSE,
            col.names=TRUE)
...
for(type in types){
  ...
  if ((type == "M") || (type == "V")){
    for(phi in 1:length(phis)){
      for(value in 1:length(values)){
        armaTime <- read.csv(paste0(inputDir, "/algo-break/break", type,
                                    "Time", values[value], "-phi", phis[phi],
                                    "-", error, ".csv"),
                             header=TRUE, sep=";")
...
write.table(summary, file=paste0(outputDir, "/algo-break-time.csv"),
            append=FALSE, quote=FALSE, sep=";", row.names=FALSE,
            col.names=TRUE)
