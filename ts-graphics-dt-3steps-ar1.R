##
## Description: Plot the results of 3 steps decision tree stationarity tests
##
...
for(error in ERROR_OPTION){
  for(p in phis){
    for(q in thetas){
      ...
      ggsave(paste0(outputDir,"/dt-3steps-arma-", error, "/stationary-ar1-" ,
  ...
  if((type == "M") | (type == "V")){
    for(test in 1:length(tests)){
      for(phi in 1:length(phis)){
        ...
        ggsave(paste0(outputDir, "/dt-3steps-trend/trend", type, "-phi",
    ...
    for(test in 1:length(tests)){
      ...
      ggsave(paste0(outputDir, "/dt-3steps-trend/trend", type, "-",
                    tests[test], ".png"),
             pl, width=9.86, height=5.43)
    }
  }
	...
	if ((type == "M") || (type == "V")){
		for(tindex in 1:length(tests)){
			for(phi in 1:length(phis)){
				data <- NULL
				for(value in 1:length(values)){
					armaResults <- read.csv(paste0(inputDir, "/dt-3steps-break/break", type,
									values[value], "-phi", phis[phi], "-",
				...
				ggsave(paste0(outputDir, "/dt-3steps-break/break", type, "-phi", phis[phi], "-",
		...
		for(tindex in 1:length(tests)){
			...
			ggsave(paste0(outputDir, "/dt-3steps-break/break", type, "-", tests[tindex], ".png"),
					pl, width=9.86, height=5.43)
		}
	}
}
