##
## Description: Summarizes the results of 2 steps decision tree stationarity tests
##
...
for(error in ERROR_OPTION){
  ...
  for(p in phis){
    for(q in thetas){
      ...
      for(tindex in 1:length(tests)){
        armaResults <- read.csv(paste0(inputDir, "/dt-2steps-arma-", error, "/arma-p",
                                       p, "-q", q, "-", tests[tindex], "-",error,
                                       ".csv"),
                                header=TRUE, sep=";")
  ...
  write.table(summary, file=paste0(outputDir, "/dt-2steps-arma-", error, "-summary.csv"),
...
for(error in ERROR_OPTION){
  ...
  for(p in phis){
    for(q in thetas){
      armaTime <- read.csv(paste0(inputDir, "/dt-2steps-arma-", error, "/armaTime-p",
                                  p, "-q", q, "-", error, ".csv"),
                           header=TRUE, sep=";")
  ...
  write.table(summary, file=paste0(outputDir, "/dt-2steps-arma-", error, "-time.csv"),
              append=FALSE, quote=FALSE, sep=";", row.names=FALSE,
              col.names=TRUE)
}
    ...
    for(tindex in 1:length(tests)){
      for(phi in 1:length(phis)){
        for(value in 1:length(values)){
          armaResults <- read.csv(paste0(inputDir,"/dt-2steps-trend/trend", type,
...
write.table(summary, file=paste0(outputDir, "/dt-2steps-trend-summary.csv"),
            append=FALSE, quote=FALSE, sep=";", row.names=FALSE,
            col.names=TRUE)
...
for(type in types){
  ...
  if((type == "M") | (type == "V")){
    for(phi in 1:length(phis)){
      for(value in 1:length(values)){
        armaTime <- read.csv(paste0(inputDir,"/dt-2steps-trend/trend", type,
...
write.table(summary, file=paste0(outputDir, "/dt-2steps-trend-time.csv"),
            append=FALSE, quote=FALSE, sep=";", row.names=FALSE,
            col.names=TRUE)
...
for(type in types){
  ...
  if((type == "M") || (type == "V")){
    for(tindex in 1:length(tests)){
      for(phi in 1:length(phis)){
        for(value in 1:length(values)){
          armaResults <- read.csv(paste0(inputDir, "/dt-2steps-break/break", type,
                                         values[value], "-phi", phis[phi], "-",
                                         tests[tindex], "-", error, ".csv"),
                                  header=TRUE, sep=";")
...
write.table(summary, file=paste0(outputDir, "/dt-2steps-break-summary.csv"),
            append=FALSE, quote=FALSE, sep=";", row.names=FALSE,
            col.names=TRUE)
...
for(type in types){
  ...
  if((type == "M") || (type == "V")){
    for(phi in 1:length(phis)){
      for(value in 1:length(values)){
        armaTime <- read.csv(paste0(inputDir, "/dt-2steps-break/break", type, "Time",
                                    values[value], "-phi", phis[phi], "-",
                                    error, ".csv"),
                             header=TRUE, sep=";")
...
write.table(summary, file=paste0(outputDir, "/dt-2steps-break-time.csv"),
            append=FALSE, quote=FALSE, sep=";", row.names=FALSE,
            col.names=TRUE)
