##
## Description: Execute individual time series stationarity tests on different
##              times of time series ARMA (NORMAL, EXP, TSTUDENT), TREND and
##              BREAK (Distributed PBS_ARRAY)
##
...
tests <- c("DF", "ADF", "PP", "SP", "ERS",
    ...
    "SC05", "SC10", "SC20", "SC30", "SC40", "SC50",
    "SC60", "SC70", "SC80", "SC90", "SC100")
...
if(PARAM_OPTION[paramID] == PARAM_ARMA){
  ...
  error <- ERROR_OPTION[errorID]
	...
	## Create directory
  tsOutputDir <- paste0(outputDir, "/", PARAM_OPTION[paramID], "-",
                        ERROR_OPTION[errorID])
  ...
  for(tindex in 1:length(tests)){
    for(windex in 1:length(windows)){
			...
			aux <- foreach(n=1:N, .combine=c) %dopar%
					runTest(tests[tindex], ts[1:windows[windex], n], alpha)
