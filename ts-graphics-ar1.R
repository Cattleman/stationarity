##
## Description: Plot the results of individual time series stationarity tests
##
## Authors: Luis Gustavo Nardin
##          Vasile Alexandru Suchar
##
## Last Modification: 10/05/2016
##
library(caTools)
library(data.table)
library(ggplot2)
...
for(error in ERROR_OPTION){
  for(p in phis){
    for(q in thetas){
      ...
      ggsave(paste0(outputDir, "/arma-", error, "/stationary-ar1-", p, "-",
                    error, ".png"),
  ...
  if((type == "M") | (type == "V")){
    for(tindex in 1:length(tests)){
      for(phi in 1:length(phis)){
        ...
        ggsave(paste0(outputDir, "/trend/trend", type, "-phi", phis[phi], "-",
    ...
    for(tindex in 1:length(tests)){
      ...
      pl <- ggplot(data, aes(x=as.numeric(as.character(window)),
        ...
        geom_line(aes(color=as.factor(as.numeric(as.character(m)))),
                  size=2) +
        xlab("Window Size") +
        ggtitle(substitute(paste(t, "     ", v),
                           list(t=tests[tindex], v=values[value]))) +
        scale_y_continuous(name="% Detected Stationarity",
                           breaks=c(0, 20, 40, 60, 80, 100),
                           labels=c("0%", "20%", "40%", "60%", "80%", "100%"),
                           limits=c(-10, 100)) +
      ...
      ggsave(paste0(outputDir, "/trend/trend", type, "-", tests[tindex],
                    ".png"), pl, width=9.86, height=5.43)
    }
  }
}
  ...
  if (type == "M"){
	...
	if ((type == "M") | (type == "V")){
        ...
        for(value in 1:length(values)){
          ...
          for(windex in 1:length(windows)){
            data <- rbind(data, cbind(values[value], windows[windex],
				...
				ggsave(paste0(outputDir, "/break/break", type, "-phi", phis[phi], "-",
								tests[tindex], ".png"), pl, width=9.86, height=5.43)
			}
		}
  } else if (type == "AC"){
		for(tindex in 1:length(tests)){
			...
			ggsave(paste0(outputDir, "/break/break", type, "-", tests[tindex], ".png"),
