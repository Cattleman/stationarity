##
## Description: Execute the stationarity algorithm on different
...
if(PARAM_OPTION[paramID] == PARAM_ARMA){
  ...
  error <- ERROR_OPTION[errorID]
	...
	## Create directory
  tsOutputDir <- paste0(outputDir, "/algo-", PARAM_OPTION[paramID], "-",
  ...
  for(tindex in 1:length(tests)){
    for(windex in 1:length(windows)){
      ...
      aux <- foreach(n=1:N, .combine=c) %dopar%
          detectStationarity(ts[1:windows[windex], n], alpha, tindex)
